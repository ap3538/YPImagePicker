...
public class YPSelectionsGalleryVC: UIViewController, YPSelectionsGalleryCellDelegate {
    ...
    override public func viewDidLoad() {
        ...
        // Add "Next" and "Manage" buttons to the navigation bar.
        let nextBarButton = UIBarButtonItem(title: YPConfig.wordings.next,
                                                style: .done,
                                                target: self,
                                                action: #selector(done))
        nextBarButton.tintColor = YPConfig.colors.tintColor
        nextBarButton.setFont(font: YPConfig.fonts.rightBarButtonFont, forState: .disabled)
        nextBarButton.setFont(font: YPConfig.fonts.rightBarButtonFont, forState: .normal)

        let manageBarButton = UIBarButtonItem(title: "Manage", style: .plain, target: self, action: #selector(manageButtonTapped))

        navigationItem.rightBarButtonItems = [nextBarButton, manageBarButton]
    ...
    @objc private func manageButtonTapped() {
        let actionSheet = UIAlertController(title: "Manage Photos", message: "Choose an option", preferredStyle: .actionSheet)

        let selectMorePhotosAction = UIAlertAction(title: "Select More Photos", style: .default) { _ in
            // Put your logic for selecting more photos here
        }
